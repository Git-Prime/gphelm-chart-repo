...
kind: Deployment
metadata:
  name: gp-pyspark-{{- template "helpers.environment.fullName" .}}-{{ .templateData.operationMode }}
  labels:
    app: gp-pyspark-{{- template "helpers.environment.fullName" .}}-{{ .templateData.operationMode }}
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: gp-pyspark-{{- template "helpers.environment.fullName" .}}-{{ .templateData.operationMode }}
          image: gp-docker.gitprime-ops.com/cloud/gp-pyspark:{{ .Values.build.commitSHA }}
...
kind: Service
metadata:
  name: gp-pyspark-{{- template "helpers.environment.fullName" .}}-{{ .templateData.operationMode }}-nodeport
  labels:
    app: gp-pyspark-{{- template "helpers.environment.fullName" .}}-{{ .templateData.operationMode }}
spec:
  type: NodePort
