...
kind: Deployment
metadata:
  name: APP_NAME-{{- template "helpers.environment.fullName" .}}-{{ .templateData.operationMode }}
  labels:
    app: APP_NAME-{{- template "helpers.environment.fullName" .}}-{{ .templateData.operationMode }}
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: APP_NAME-{{- template "helpers.environment.fullName" .}}-{{ .templateData.operationMode }}
          image: gp-docker.gitprime-ops.com/cloud/APP_NAME:{{ .Values.build.commitSHA }}
...
kind: Service
metadata:
  name: APP_NAME-{{- template "helpers.environment.fullName" .}}-{{ .templateData.operationMode }}-nodeport
  labels:
    app: APP_NAME-{{- template "helpers.environment.fullName" .}}-{{ .templateData.operationMode }}
spec:
  type: NodePort
